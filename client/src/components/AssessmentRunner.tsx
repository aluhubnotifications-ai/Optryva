...
import { ProctorMonitor, VIOLATION_LABEL, type ProctorViolation } from '@/components/ProctorMonitor'
...
function QuestionField({ question, value, onChange }: { question: any; value?: string | string[]; onChange: (value: string | string[]) => void }) {
  ...
  return (
    <div className="rounded-xl border border-border p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        {question.required && <span className="shrink-0 rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">Required</span>}
      </div>
      {question.type === 'essay' ? (
        <div className="mt-3">
          <textarea
          ...
          {(question.minWords || question.maxWords) && (() => {
            const wc = (typeof value === 'string' ? value : '').trim().split(/\s+/).filter(Boolean).length
            ...
            return (
              <p className={`mt-1 text-xs ${over || under ? 'text-danger' : 'text-muted-foreground'}`}>
                {wc} words{question.maxWords ? ` / ${question.maxWords} max` : ''}{question.minWords ? ` · ${question.minWords} min` : ''}
          ...
          {choices.map((choice) => (
            <label key={choice} className="flex cursor-pointer items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm hover:border-accent">
              <input
                ...
                onChange={() => (question.type === 'multiple_choice' ? onChange(selected.includes(choice) ? selected.filter((i: string) => i !== choice) : [...selected, choice]) : onChange(choice))}
...
export function AssessmentRunner({ job, application, onComplete, onClose }: { job: JobListing; application: Application; onComplete: (app: Application) => void; onClose: () => void }) {
  const { toast } = useToast()
  const assignment = job.assignment!
  const questions = (assignment.questions?.length ? assignment.questions : assignment.rubric.map((c) => ({ id: c.id, type: 'essay' as const, prompt: c.label, required: true }))) as any[]
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      setRemaining((s) => {
        if (s <= 1) { clearInterval(iv); setTimeUp(true); return 0 }
        return s - 1
      })
    }, 1000)
  ...
  async function submitTest() {
    setSubmitting(true)
    const duration_seconds = Math.round((Date.now() - startedAt.current) / 1000)
    const assignment_answers = questions.map((q) => ({ question_id: q.id, answer: answers[q.id] ?? '' }))
    try {
      const updated = await applicationsApi.submitAssignment(application.id, { assignment_answers, duration_seconds })
      toast({ title: 'Test submitted', tone: 'success' })
      onComplete(updated)
    } catch (e) {
      toast({ title: 'Could not submit test', description: e instanceof Error ? e.message : undefined, tone: 'error' })
    } finally {
      setSubmitting(false)
    }
  }
    ...
    return (
      <div className="space-y-4 rounded-2xl border border-danger/30 bg-danger/5 p-6 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          {canRetry ? `Attempt ${used} of ${maxAttempts}. You can take the test again.` : `You've used all ${maxAttempts} attempts for this test.`}
          {timeUp ? ' The time limit was also reached.' : ''}
        </p>
        {canRetry ? (
          <Button className="mt-3" onClick={() => { setProctorCancelled(null); setConsentGiven(false); setConsentChecked(false); setAnswers({}) }}>Try again</Button>
        ) : (
    ...
    return (
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        <label className="mt-4 flex items-start gap-2 text-sm">
          ...
          <span>I agree to be monitored by camera and microphone for the duration of this test, and I understand the test will be cancelled if these rules are broken.</span>
        </label>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-accent/30 bg-accent/5 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className={cn('flex shrink-0 items-center gap-1 rounded-full px-3 py-1 text-sm font-medium', timeUp ? 'bg-danger/15 text-danger' : 'bg-muted text-muted-foreground')}>
            <Lock className="h-3.5 w-3.5" /> {fmt(remaining)}{timeUp ? ' — time up, submit now' : ` / ${durationMin}m`}
          </div>
        </div>
        <div className="mt-4 space-y-3">
          {questions.map((question) => (
            <QuestionField key={question.id} question={question} value={answers[question.id]} onChange={(value) => setAnswers((c) => ({ ...c, [question.id]: value }))} />
          ))}
        </div>
        <Button className="mt-4 w-full" onClick={submitTest} loading={submitting}>Submit test</Button>
      </div>
    </div>
  )
}
