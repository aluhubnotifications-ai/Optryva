...
import { FileText, ShieldCheck, Sparkles, ArrowRight, Users, Eye, ExternalLink } from 'lucide-react'
      ...
      {loading ? (
        ...
        <>
          <div className="-mx-1 flex flex-wrap items-center gap-1.5 overflow-x-auto px-1">
            ...
            <select value={listingFilter} onChange={(e) => setListingFilter(e.target.value)} className="ml-auto rounded-lg border border-border bg-card px-2 py-1.5 text-sm">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="space-y-3">
              {filtered.map((a) => {
                ...
                return (
                  <button key={a.id} onClick={() => navigate(`/app/applicants/${a.id}`)} className="block w-full text-left">
